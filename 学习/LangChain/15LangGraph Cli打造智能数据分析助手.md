实现三个功能：NL2sql,NL2python,tavily
创建两个外部函数，sql_inter执行mysql数据查询和extract_data数据库的数据提取或取部分数据保存到本地csv文件。分成两个外部函数会更加稳定一些，因为要传参数
python代码解释器函数编写也是有两个外部函数。python_inter用来执行普通python代码，fig_inter绘图类的代码
修改requirements.txt
```Plaintext
langgraph
langchain-core
...
```
在langgraph.json文件里面把agent名修改为data_agent.

修改graph.py文件。然后运行后端再前端
```
langgraph dev
```
![[Pasted image 20250714114919.png]]
![[Pasted image 20250714115433.png]]
```
cd agent-chat-ui
pnpm dev
```
![[Pasted image 20250714145404.png]]
![[Pasted image 20250714145440.png]]
让他进行数据分析，绘制图表
![[Pasted image 20250714145555.png]]
我还下载了telco_data.csv文件，是非常有名的电信用户流失预测数据集
```Python
import os
...
def fig_inter(py_code: str, fname: str) -> str:
    ...
    # ✅ 设置图像保存路径（你自己的绝对路径）public专门放静态资源
    base_dir = r"D:\LangChainlianxi\agent-chat-ui\public"
