##### 搜索函数
尽可能地去搜索和我的问题相关的知识。给出关键词后调用搜索引擎去搜索，搜索之前呢会根据我的关键词生成四个不同的问题，然后使用这些问题去搜索，获取每个URL，对URL进行去重，然后利用https://r.jina.ai/去获取网页的具体内容，通过大模型去判断网页内容对我的问题是否有帮助，有帮助yes就提取有用的部分，没有帮助就舍弃，对提取出来的内容进行存储，然后进行判断内容是否已经可以对我的关键词和生成的问题进行处理，若是不能，则会生成新的问题，继续收集。当收集的知识足够回答我的问题或达到迭代次数就会停止收集。然后返回所有搜索结果。
##### 获取图片链接函数
获取完图片链接，会通过多模态的模型对图片的内容进行总结，方便在写报告的时候在合适的地方插入这张图片
```python
...
api_key = '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'
...
    links = []
    response = requests.get(f'http://192.168.1.128:8080/search?format=json&q={query}&language=zh-CN&time_range=&safesearch=0&categories={categories}', timeout=10)
